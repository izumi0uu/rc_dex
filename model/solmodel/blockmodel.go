...
func (m *defaultBlockModel) FindLastSuccessBlock(ctx context.Context) (*Block, error) {
	var resp Block
	err := m.conn.WithContext(ctx).Model(&Block{}).Where("`status` = ?", constants.BlockProcessed).
		Order("slot desc").First(&resp).Error
